...
<?php
  ...
  if( !empty($_POST) ){     // processar o pedido
    ...
    if( mysql_num_rows($q) == 1 ){
      ...
      $chave = sha1(uniqid( mt_rand(), true));  // guardar este par de valores na tabela para confirmar mais tarde
      ...
      if( mysql_affected_rows() == 1 ){
        ...
        if( mail($user, 'Recuperação de password', 'Olá '.$user.', acesse este link '.$link) 
        
        {
          echo '<p >Foi enviado um e-mail para o seu endereço, onde você encontrará um link para alterar a sua Senha</p>';
